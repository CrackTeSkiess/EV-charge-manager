...
class EnergyManagerAgent:
    ...
    def interpret_action(
        ...
        if battery_target_kw > 0:  # Want to charge
            ...
            # Discharge to meet remaining demand, bounded by available SOC
            available_discharge_kwh = max(
                0.0, (self.current_soc - 0.1) * self.battery_capacity_kwh
            )
            max_discharge_kw = min(self.battery_max_power_kw, available_discharge_kwh)
            discharge_needed = min(-battery_target_kw, remaining_demand, max_discharge_kw)
