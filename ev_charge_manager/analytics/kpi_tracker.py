...
from typing import List, Dict, Optional, Callable, Any, Tuple, Set, TYPE_CHECKING
...
if TYPE_CHECKING:
    from ev_charge_manager.vehicle import VehicleTracker
...
class KPITracker:
    ...
    def __init__(
        ...
        # Vehicle tracker reference (set externally or None)
    ...
    def record_step(
        ...
        # FIX: Properly count strandings instead of hardcoding 0
        cars_zero_battery = self._count_strandings(environment, step_events)
        ...
        # Get PER-STEP values (not cumulative!) from environment's step_metrics
        # Priority 1: Use VehicleTracker if available (most accurate)
        if self.tracker:
            step_metrics = self.tracker.get_step_metrics()
            vehicles_entered = step_metrics.vehicles_spawned
            vehicles_exited = step_metrics.vehicles_completed
            vehicles_stranded = step_metrics.vehicles_stranded
        # Priority 2: Use environment's step_metrics
        elif hasattr(environment, 'step_metrics'):
            vehicles_entered = environment.step_metrics.get('spawned_this_step', 0)
            vehicles_exited = environment.step_metrics.get('completed_this_step', 0)
            vehicles_stranded = environment.step_metrics.get('stranded_this_step', 0)
        # Priority 3: Fallback (will be cumulative - not ideal)
        else:
        ...
        self._total_stranded += vehicles_stranded
        ...
        # Efficiency - use total entered for this step, or use rolling window
        active_vehicles = max(1, vehicles_on_road)
        abandonment_rate = (
            cars_quit_waiting / max(1, active_vehicles)
            if step > 0 else 0.0
        )

        # Service level (vehicles served within acceptable wait)
        service_level = 1.0 - min(1.0, abandonment_rate)
    ...
    def _count_strandings(self, environment: Any, step_events: Dict) -> int:
        """
        Count vehicles that became stranded (0% battery) this step.
        Uses multiple sources to ensure accurate counting.
        """
        # Priority 1: Use VehicleTracker if available (most reliable)
        if self.tracker:
            step_metrics = self.tracker.get_step_metrics()
            return step_metrics.vehicles_stranded

        # Priority 2: Count from step_events (highway reports strandings)
        strandings_from_events = len(step_events.get('strandings', []))
        if strandings_from_events > 0:
            return strandings_from_events

        # Priority 3: Use environment's current_metrics
        return environment.current_metrics.get('vehicles_stranded', 0)
    ...
    def _count_quit_waiting(self, highway: Any, step_events: Dict) -> int:
        ...
        """
        # Priority 1: Use VehicleTracker if available
        if self.tracker:
            step_metrics = self.tracker.get_step_metrics()
            return step_metrics.vehicles_abandoned_queue

        # Priority 2: Direct from events
