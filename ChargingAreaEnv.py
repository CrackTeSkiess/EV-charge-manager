...
from HierarchicalEnergyManager import HierarchicalEnergyManager, GridPricingSchedule
...
class MultiAgentChargingEnv(gym.Env):
    ...
    def _decode_actions(self, actions: np.ndarray) -> Tuple[
        ...
        for i, action in enumerate(actions):
            ...
            if i < self.n_agents - 1:
                pos = min(pos, self.highway_length_km - 10)
            ...
            # Create hierarchical manager with micro-RL agent
    ...
    def _run_simulation_with_micro_rl(
        ...
        # Count shortages (when grid couldn't meet demand after battery empty)
        shortage_events = 0
        for manager in managers:
            for h in manager.hourly_history:
                if h['grid_used'] > 0 and h['battery_soc'] <= 0.11:  # Grid used with empty battery
                    shortage_events += 1
    ...
    def _compute_rewards(
        ...
        info = {
            ...
            'renewable_fraction': sim_results['total_renewable_kwh'] / 
                max(1, sim_results['total_renewable_kwh'] + sim_results['total_grid_energy_kwh']),
        }
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        self.weather = WeatherProfile(
            min_solar=float(random.uniform(0.0, 0.2)),
            max_solar=float(random.uniform(0.6, 1.0)),
            solar_variance=float(random.uniform(0.05, 0.2)),
            min_wind=float(random.uniform(0.0, 0.3)),
            max_wind=float(random.uniform(0.4, 1.0)),
            wind_variance=float(random.uniform(0.1, 0.3)),
        )
    ...
    def _get_observations(self) -> np.ndarray:
        ...
        for idx in sorted_indices:
            ...
            if idx > 0:
                left_dist = agent.position - self.agents[sorted_indices[idx-1]].position
                obs[4] = left_dist / self.highway_length_km
            else:
                obs[4] = agent.position / self.highway_length_km
            
            if idx < self.n_agents - 1:
                right_dist = self.agents[sorted_indices[idx+1]].position - agent.position
                obs[5] = right_dist / self.highway_length_km
            else:
