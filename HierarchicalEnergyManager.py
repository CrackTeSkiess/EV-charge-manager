...
class HierarchicalEnergyManager:
    ...
    def _update_stats(self, flows: Dict, grid_price: float, dt_hours: float):
        ...
        if flows['battery_power'] < 0:  # Discharging
            # Value of energy discharged at current price
            self.daily_stats['arbitrage_profit'] += abs(flows['battery_power']) * dt_hours * grid_price
